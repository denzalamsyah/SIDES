...
if($op == 'edit'){
  $id          = $_GET['id'];
  $sql2        = "SELECT * FROM tb_pemasaran where id = '$id'";
  $q2          = mysqli_query($conn, $sql2);
  $r2          = mysqli_fetch_array($q2);
  $nik         = $r2['NIK'];
  $nama_bisnis = $r2['Nama_Bisnis'];
...
if (isset($_POST['submit'])) {
  ...
  if($nik && $nama_bisnis && $deskripsi){
    ...
    if($op == 'edit'){ //untuk update
      $sql2 = "UPDATE tb_pemasaran set NIK = '$nik', Nama_Bisnis = '$nama_bisnis', Deskripsi = '$deskripsi' where id = '$id'";
      $q2   = mysqli_query($conn, $sql2);
      if($q2){
        header("Location: datapemasaran.php");
      }else{
      ...
      $result2 = mysqli_query($conn, $sql2);
           ...
           if ($result2) {
            $success ="Anda Berhasil memasukan data";
	          ...
	          $error="Anda Gagal memasukan data";
           }
    }
}else{
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="datapemasaran.php" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Lihat Pemasaran</p>
                </a>
              </li>
